...
# Shortcut 2: Claude Code (No Permissions) - skips permission prompts
$s2 = $WshShell.CreateShortcut("$desktop\Claude Code (No Permissions).lnk")
$s2.TargetPath = $mintty
$s2.Arguments = "-i /mingw64/share/git/git-for-windows.ico -e /usr/bin/bash --login -c ""source ~/.bashrc 2>/dev/null; cd '$projBash' 2>/dev/null; claude --dangerously-skip-permissions; exec bash -i"""
$s2.WorkingDirectory = $projWin
$s2.Description = "Claude Code with Hebrew RTL - no permission prompts"
if ($claudeIcon) { $s2.IconLocation = $claudeIcon }
$s2.Save()

# Shortcut 3: Git Bash Hebrew - uses Git icon
$s3 = $WshShell.CreateShortcut("$desktop\Git Bash Hebrew.lnk")
$s3.TargetPath = $mintty
$s3.Arguments = "-i /mingw64/share/git/git-for-windows.ico -e /usr/bin/bash --login -i"
$s3.WorkingDirectory = $projWin
$s3.Description = "Git Bash with Hebrew RTL support"
if ($gitIcon) { $s3.IconLocation = $gitIcon }
$s3.Save()
...
PSEOF
    ...
    if [ -f "$DESKTOP_UNIX/Claude Code (No Permissions).lnk" ]; then
        ok "Created: Claude Code (No Permissions)"
    else
        warn "Could not create Claude Code (No Permissions) shortcut"
    fi
...
echo "    Claude Code (No Permissions) Opens Claude without permission prompts"
